#ifndef FORTRAN_TESTING_PRESENTATION_MUTUAL_INFORMATION_CUDA_IMPL_CUH
#define FORTRAN_TESTING_PRESENTATION_MUTUAL_INFORMATION_CUDA_IMPL_CUH
...
struct CudaArray {
    ...
    explicit CudaArray(const size_t n_elements)
        : bytes(n_elements * sizeof(T)) {
...
__device__ void warp_reduce_kernel(volatile int* s_a, const int l_tidx) {
...
__device__ void min_warp_reduce_kernel(
    volatile double* s_min,
    volatile int* s_min_idx,
    const int l_tidx
) {
...
void launch_kernel(
    ...
    ksg_counts_kernel<LocalMSize>
        <<<grid_dim_x, block_dim_x>>>(
            d_mx, d_my, k, n_points,
            d_mx_counts, d_my_counts
        );
}
...
void cuda_ksg_counts::operator()(
    std::span<const double> Mx,
    std::span<const double> My,
    int n_points,
    int k,
    std::span<int> mx_counts,
    ...
    dispatch_kernel(
        config.local_m_size,
        std::make_integer_sequence<int, 101>{},
        cuda_mx.d_array,
        cuda_my.d_array,
        k,
        n_points,
        cuda_mx_counts.d_array,
        cuda_my_counts.d_array,
        config.grid_dim_x,
        config.block_dim_x
    );

    cuda_mx_counts.to_host(mx_counts.data());
    cuda_my_counts.to_host(my_counts.data());
}

} // namespace ksg

#endif // CUDA_SUPPORT
#endif // FORTRAN_TESTING_PRESENTATION_MUTUAL_INFORMATION_CUDA_IMPL_CUH
