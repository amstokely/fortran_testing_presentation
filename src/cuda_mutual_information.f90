module cuda_mutual_information
   use iso_c_binding
   use mutual_information
   implicit none
   private
   public :: cuda_ksg_counts

   interface
      subroutine cuda_ksg_counts_i(Mx, My, n_points, k, mx_counts, my_counts, err) &
            bind(C, name = "c_cuda_ksg_counts")

         import :: c_double, c_int

         real(c_double), intent(in) :: Mx(*)
         real(c_double), intent(in) :: My(*)
         integer(c_int), value :: n_points
         integer(c_int), value :: k
         integer(c_int), intent(out) :: mx_counts(*)
         integer(c_int), intent(out) :: my_counts(*)
         integer(c_int), intent(out) :: err
      end subroutine
   end interface

contains

   subroutine cuda_ksg_counts(Mx, My, k, mx_counts, my_counts, err)
      use iso_fortran_env, only : real64
      implicit none
      real(real64), intent(in) :: Mx(:)
      real(real64), intent(in) :: My(:)
      integer, intent(in) :: k
      integer, intent(out) :: mx_counts(size(Mx)), my_counts(size(Mx))
      type(error_t), intent(out) :: err

      integer(c_int) :: c_mx_counts(size(Mx)), c_my_counts(size(Mx))
      integer(c_int) :: c_err

      call validate_ksg_parameters(Mx, My, k, err)
      if (err%code /= 0) then
         return
      end if

      call cuda_ksg_counts_i(&
            Mx, &
            My, &
            size(Mx), &
            k, &
            c_mx_counts, c_my_counts, c_err)
      call handle_ksg_status(c_err, err)
      if (err%code /= 0) then
         return
      end if
      mx_counts = c_mx_counts
      my_counts = c_my_counts
   end subroutine

end module