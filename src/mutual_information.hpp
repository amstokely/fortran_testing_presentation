...
namespace ksg {
    ...
    max_norm_from_point(std::span<const double> Mx, std::span<const double> My,
                        double xref_x, double xref_y) {
        const std::size_t n = Mx.size();
        std::vector<double> dists(n);

        for (std::size_t i = 0; i < n; ++i) {
            dists[i] = std::max(std::abs(Mx[i] - xref_x),
                                std::abs(My[i] - xref_y));
        }
    ...
    max_neighbor_distance(std::span<const double> X, double xref,
        ...
        double max_dist = 0.0;
        for (auto i: idxs) max_dist = std::max(max_dist, std::abs(X[i] - xref));
        return max_dist;
    }
    ...
    // -------------------------------------------------------------------------
    inline std::size_t
    count_neighbors_within_radius(std::span<const double> X, double xref,
                                  double radius) {
    ...
    inline std::array<std::size_t, 2>
        cpp_ksg_count(const std::span<const double> Mx,
            ...
            auto max_norm = [&](std::size_t i) {
                return std::max(std::abs(Mx[i] - xref_x), std::abs(My[i] - xref_y));
            };
    ...
    struct cpp_ksg_counts {
        void operator()(const std::span<const double> Mx,
                                  const std::span<const double> My, const int n_points, const int k,
                                  int *nx, int *ny) {
    ...
    void ksg_counts(const std::span<const double> Mx,
                              const std::span<const double> My, const int n_points, const int k,
                              int *nx, int *ny) {
...
// -----------------------------------------------------------------------------
extern "C" {
void c_cpp_ksg_count(const double *Mx, const double *My, int n_points,
                     int ref_idx, // 0-based
                     int k, int &nx, int &ny);
