...
namespace ksg {
    ...
    max_norm_from_point(
        ...
        for (std::size_t i = 0; i < n; ++i) {
            dists[i] = std::max(
                std::abs(Mx[i] - xref_x), std::abs(My[i] - xref_y)
            );
        }
    ...
    max_neighbor_distance(
        std::span<const double> X, double xref,
    ...
    count_neighbors_within_radius(
        std::span<const double> X, double xref, double radius
    ) {
    ...
    template<typename KsgCountsStrategy = cpp_ksg_counts> void
    ksg_counts(
        const std::span<const double> Mx,
        const std::span<const double> My, const int n_points,
        const int k, int *mx_counts, int *my_counts
